...
-- Kör dessa i Supabase SQL Editor för att verifiera att live-DB matchar detta schema.
-- OBS: Query A har många rader — klicka Export i Supabase UI för att få alla.
-- Uppdatera detta schema + SYSTEM_ARCHITECTURE.md om det finns avvikelser.
-- Senast verifierat: 2026-02-18

-- A. ALLA TABELLER + KOLUMNER
-- OBS: Kan returnera 200+ rader. Klicka Export (CSV) i Supabase UI för att se alla.
SELECT
  t.table_name,
...
-- C. FOREIGN KEYS (relationer + CASCADE-regler)
SELECT
...
-- D. INDEX (alla utom primärnycklar)
SELECT
  tablename,
  indexname,
  indexdef
...
-- E. ROW LEVEL SECURITY — vilka tabeller har RLS aktiverat
SELECT
  tablename,
...
-- G. POSTGRESQL FUNKTIONER / STORED PROCEDURES
SELECT
  routine_name,
  routine_type,
...
-- I. CHECK-CONSTRAINTS (allowed values / enums)
SELECT
  tc.table_name,
  tc.constraint_name,
  cc.check_clause
...
-- J. RADANTAL PER TABELL (datahälsa)
SELECT
